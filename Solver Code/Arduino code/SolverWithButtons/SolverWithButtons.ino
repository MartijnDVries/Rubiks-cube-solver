...
char cubeArray[20][6] = { {'g','r','-','-','w','-'},
  ...
  // Button pins
  const int solveButtonPin = A1;
  const int scrambleButtonPin = A2;
  int solveButtonState = 0;
  int scrambleButtonState = 0;

  // Led pins
  const int solveLedPin = A3;
  const int scrambleLedPin = A4;
...
void setup() {
  ...
  pinMode(solveButtonPin, INPUT);
  pinMode(scrambleButtonPin, INPUT);
  pinMode(solveLedPin, OUTPUT);
  pinMode(scrambleLedPin, OUTPUT);
